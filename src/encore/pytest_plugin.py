"""pytest plugin: zero-config `encore_cassette` fixture.

Loading: declared via `[project.entry-points."pytest11"]` in pyproject.toml -
pytest discovers it automatically when both pytest and encore are installed.
...
Or per-test override:
    ...
    @pytest.mark.encore(path="custom.yaml", mode="replay_only")
    def test_my_agent(encore_cassette):
        ...
"""
...
from encore.api import cassette
from encore.session import Session


def pytest_configure(config):
    config.addinivalue_line(
        "markers",
        "encore(path, mode, match_on): customize the cassette for this test",
    )


@pytest.fixture
def encore_cassette(request) -> Session:
    """Open a cassette for the current test.

    Defaults:
      - path: tests/cassettes/<module>/<test_name>.yaml
      - mode: ENCORE_DEFAULT_MODE env var, else record_new
    """
    marker = request.node.get_closest_marker("encore")
